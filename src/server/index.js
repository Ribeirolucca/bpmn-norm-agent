...
import { generateBpmn, improveBpmnLayout, interviewProcess } from "./openaiAgent.js";
...
app.post("/api/improve-layout", async (req, res) => {
  try {
    const bpmnXml = String(req.body?.bpmnXml || "").trim();
    if (bpmnXml.length < 100) {
      res.status(400).json({ error: "Envie um BPMN XML valido para melhorar." });
      return;
    }
    res.json(await improveBpmnLayout({ bpmnXml }));
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
app.listen(port, host, () => {
  console.log(`BPMN Norm Agent API em http://${host}:${port}`);
});
